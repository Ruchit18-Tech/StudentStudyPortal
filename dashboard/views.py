...
def notes(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            notes = Notes(user=request.user, title=form.cleaned_data["title"],
...
def homework(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                finished = False
            homework = Homework(user=request.user, subjects=form.cleaned_data["subjects"],
    ...
    if len(homework) == 0:
        ...
        homework_done = False
    context = {'homeworks': homework, 'homewrok_done': homework_done, 'form': form}
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                finished = False
            todo = Todo(user=request.user, title=form.cleaned_data['title'], is_finished=finished)
            todo.save()
            messages.success(request, f"Todos Added from {request.user.username} Sucessfully!")
    else:
        form = TodoForm()
